...
5. `AlertManager` проверяет dedupe в Redis и создаёт in-memory snapshot opportunity только если изменение прошло порог.
6. `FanoutManager` подбирает Telegram-получателей по пользовательским фильтрам и собирает in-memory delivery.
7. Worker ревалидирует свежие книги, сразу пытается отправить delivery и после успешной обработки обновляет dedupe-state в Redis. Постоянная запись opportunities/alerts в PostgreSQL не используется.
