...
7. Worker быстро создаёт `Alert` и коммитит его, а Telegram loop добирает queued/retry `Alert` из БД; в режиме `worker` без Telegram runtime остаётся immediate-send fallback. Recovery `fanout` loop при отдельном запуске разбирает queued/retry `ArbOpportunity` батчами.
...
`all` рассчитан на основной сценарий, где worker не блокируется на Telegram-отправке: он создаёт queued-alerts, а отдельный Telegram loop доставляет их параллельно.
